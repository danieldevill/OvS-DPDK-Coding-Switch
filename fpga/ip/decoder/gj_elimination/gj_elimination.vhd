...
architecture rtl of gj_elimination is
	...
	signal pkt_coef_in_rowDiv : std_logic_vector(55 downto 0);
	...
	signal pkt_coef_i_in_mulsub : std_logic_vector_array_mulsub_coeffs;
	signal pkt_coef_j_in_mulsub : std_logic_vector_array_mulsub_coeffs;
	signal pkt_data_i_in_mulsub : std_logic_vector_array_mulsub_data_in;
	signal pkt_data_j_in_mulsub : std_logic_vector_array_mulsub_data_in;
	signal pkt_coef_out_mulsub : std_logic_vector_array_mulsub_coeffs;
	signal pkt_data_out_mulsub : std_logic_vector_array_mulsub_data_in;
...
begin
	...
	generate_gj_mulsubRows: for i in 0 to (h-1) generate 
		...
		port map (
			...
			rst => rst_mulsub,
			a => a_mulsub(i),
			pkt_coef_i_in => pkt_coef_i_in_mulsub(i),
			pkt_coef_j_in => pkt_coef_j_in_mulsub(i),
			pkt_data_i_in => pkt_data_i_in_mulsub(i),
			pkt_data_j_in => pkt_data_j_in_mulsub(i),
			pkt_coef_out => pkt_coef_out_mulsub(i),
			pkt_data_out => pkt_data_out_mulsub(i),
	...
	port map (
		...
		rst => rst_rowDiv,
		a => a_rowDiv,
		pkt_coef_in => pkt_coef_in_rowDiv,
		pkt_data_in => pkt_data_in_rowDiv,
		pkt_coef_out => pkt_coef_out_rowDiv,
		pkt_data_out => pkt_data_out_rowDiv,
		...
		if rst = '0' then
			...
			if gj_elimination_state = load then
				if(load_count = h) then
					...
					for i in 0 to (h-1) loop
						coeff_matrix(load_count)(i) <= pkt_coef_in( ((i*m)+(m-1)) downto ((i*(m-1))+i) );
					end loop;
					for i in 0 to (N-1) loop
						data_matrix(load_count)(i) <= pkt_data_in( ((i*m)+(m-1)) downto ((i*(m-1))+i) );
					end loop;
				end if;
			else
				load_count <= 0;
			...
			if gj_elimination_state = divRow then
				if(div_done = '1') then
					...
					for i in 0 to (h-1) loop
						coeff_matrix(lower_triangle_count)(i) <= pkt_coef_out_rowDiv( ((i*m)+(m-1)) downto ((i*(m-1))+i) );
					end loop;
					for i in 0 to (N-1) loop
						data_matrix(lower_triangle_count)(i) <= pkt_data_out_rowDiv( ((i*m)+(m-1)) downto ((i*(m-1))+i) );
					end loop;
				end if;
			end if;
					...
					if lower_triangle = '1' then
						if lower_triangle_count = 1 then
							for i in 1 to (h-1) loop
								for j in 0 to (h-1) loop
									coeff_matrix(i)(j) <= pkt_coef_out_mulsub(i)( ((j*m)+(m-1)) downto ((j*(m-1))+j) );
								end loop;
								for j in 0 to (N-1) loop
									data_matrix(i)(j) <= pkt_data_out_mulsub(i)( ((j*m)+(m-1)) downto ((j*(m-1))+j) );
								end loop;
							end loop;
			...
			if gj_elimination_state = complete then
				if(complete_count = h) then
					...
					for i in 0 to (h-1) loop
						pkt_coef_out( ((i*m)+(m-1)) downto ((i*(m-1))+i) ) <= coeff_matrix(complete_count)(i);
					end loop;
					for i in 0 to (N-1) loop
						pkt_data_out( ((i*m)+(m-1)) downto ((i*(m-1))+i) ) <= data_matrix(complete_count)(i);
					end loop;
				end if;
			else
				complete_count <= 0;
	...
	begin
		if rst = '0' then
			...
			if gj_elimination_state = divRow then
				...
				--Set rows to divide
				for i in 0 to (h-1) loop
					pkt_coef_in_rowDiv( ((i*m)+(m-1)) downto ((i*(m-1))+i) ) <= coeff_matrix(lower_triangle_count)(i);
				end loop;
				for i in 0 to (N-1) loop
					pkt_data_in_rowDiv( ((i*m)+(m-1)) downto ((i*(m-1))+i) ) <= data_matrix(lower_triangle_count)(i);
				end loop;
				...
				if lower_triangle = '1' then
					...
					if lower_triangle_count = 0 then
						for i in 1 to (h-1) loop
							...
							for j in 0 to (h-1) loop
								pkt_coef_i_in_mulsub(i)( ((j*m)+(m-1)) downto ((j*(m-1))+j) ) <= coeff_matrix(lower_triangle_count)(j);
								pkt_coef_j_in_mulsub(i)( ((j*m)+(m-1)) downto ((j*(m-1))+j) ) <= coeff_matrix(i)(j);
							end loop;
							for j in 0 to (N-1) loop
								pkt_data_i_in_mulsub(i)( ((j*m)+(m-1)) downto ((j*(m-1))+j) ) <= data_matrix(lower_triangle_count)(j);
								pkt_data_j_in_mulsub(i)( ((j*m)+(m-1)) downto ((j*(m-1))+j) ) <= data_matrix(i)(j);
							end loop;
						end loop;
				...
				if upper_triangle = '1' then
					...
					if upper_triangle_count = 0 then
						for i in 0 to 1 loop
							...
							for j in 0 to (h-1) loop
								pkt_coef_i_in_mulsub(i)( ((j*m)+(m-1)) downto ((j*(m-1))+j) ) <= coeff_matrix(upper_triangle_count+1)(j);
								pkt_coef_j_in_mulsub(i)( ((j*m)+(m-1)) downto ((j*(m-1))+j) ) <= coeff_matrix(i)(j);
							end loop;
							for j in 0 to (N-1) loop
								pkt_data_i_in_mulsub(i)( ((j*m)+(m-1)) downto ((j*(m-1))+j) ) <= data_matrix(upper_triangle_count+1)(j);
								pkt_data_j_in_mulsub(i)( ((j*m)+(m-1)) downto ((j*(m-1))+j) ) <= data_matrix(i)(j);
							end loop;
						end loop;
